...
secret **scalar**, the ML-KEM-768 post-quantum KEM primitive
([`../../libsigil/core/src/mlkem.rs`](../../libsigil/core/src/mlkem.rs)) needs a
64-byte **keygen seed** (`d || z`) and a 32-byte **encapsulation coin** (`m`), and
the ML-DSA-65 post-quantum signature primitive
([`../../libsigil/core/src/mldsa.rs`](../../libsigil/core/src/mldsa.rs)) needs a
32-byte **keygen seed** (the FIPS 204 `xi`). The question is *who* generates that
entropy.
...
- the **ML-DSA-65 keygen seed** (the FIPS 204 `xi`, 32 bytes) — signing itself is
  deterministic per FIPS 204 (a zero randomizer is permitted), so no per-signature
  RNG is needed either.
...
- **Pre-audit reality:** the primitives that consume this entropy (Argon2id, the
  AEAD, the standalone Ed25519 sign/verify, the standalone ML-DSA-65 sign/verify,
  the standalone X25519 key-agreement, and the standalone ML-KEM-768 KEM) are
  **real but UNAUDITED** and not wired into a finished product; the two hybrid-KEM
  halves are now combined by the standalone hybrid KEM combiner
  ...
  coin), while the two hybrid-signature halves (Ed25519 and ML-DSA-65) exist
  standalone but are **not yet combined** into a hybrid `Sign`/`Verify`. See
