...
| `deploy/local/compose.yaml` | **Brought up on loopback and probed**, then torn down — local Caddy → `sigild` topology smoke ([§9](#9-local-topology-check)). | **Local shape check only.** Caddy reverse-proxies to `sigild` on `127.0.0.1`; probes answered through the edge. **No real TLS/ACME, no PQ proof, not exposed.** Disposable — not a deployment. |
