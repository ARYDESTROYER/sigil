...
| [0051](0051-provisioning-bounds-and-qr-ingest.md) | **Bounds on an untrusted provisioning payload, and the QR door that made them urgent** — `otpauth://…&period=4294967295` was accepted and stored, producing a code that NEVER CHANGES (measured: **755224 at t=59, t=1.9e9 and t=4e9**) rendered with an **ordinary-looking countdown**, so the user believes they are protected by a static secret in a rotating costume. ⭐ **Every existing door had a human reading the URI; a QR is opaque to humans by construction, so it removes the last reviewer** — which is why the parser was hardened BEFORE the scanner shipped. Six bounds single-sourced in `sigil-core` and mirrored once in JS (period <= 600, secret <= 1024 B, label/issuer <= 256 code points, no C0/C1 or bidi-override characters — ordinary RTL script untouched — digits 6..=10, <= 512 accounts per payload, the last chosen against `sigild`' own 64 KiB op cap). ⭐⭐ **The ceiling is INGEST-ONLY, reusing [0047](0047-container-parameter-ceiling-and-no-downgrade-ratchet.md)' rule: refusing to ADD is a different act from refusing to READ**, so a vault already holding an out-of-bounds entry still opens and still generates its codes, with a read-path warning that ⛔ **reports and never corrects**; and there is deliberately **NO FLOOR** (a two-character secret is pinned as accepted). ⛔ **Honest limits:** `sigil totp add --period N` is deliberately ungated (a number the operator typed is not a stranger' payload), the Phase 61 **vault merge is deliberately not gated** (refusing to merge an entry is refusing to READ it — the data-loss direction [0049](0049-entry-identity-and-the-mergeable-vault.md) exists to repair), 512 accounts can still produce a vault too big to sync, the bounds are **not retroactive**, and `BarcodeDetector` is **secure-context gated** so a plain-HTTP LAN origin silently gets no scanner. ⭐ **`sigild` changed by zero lines and nothing new was added to any dependency file** | Accepted (2026-08) |
| [0052](0052-recovery-discovery-and-the-printed-sheet.md) | **Recovery discovery, the printed sheet, and the stranger's vault** — the one flow that exists because everything else is already lost could **silently restore a partial vault set and report success** (the Rust CLI never read `has_more`; the JS half has refused since Phase 58), and a **third party could deny discovery outright** by crowding the single uncursored 500-row page of `GET /v1/devices/{deviceID}/keys` (**measured: 520 vaults in 0.6 s with junk bytes, ~3 s when every envelope is genuine and authenticated**; the kit's unguessable device id is disclosed to any `read` collaborator by `GET /v1/vaults/{id}/grants`, permanently). ⭐ **The answer was already on paper**: the printed sheet's `covers` line names the vault ids, and a restore given them asks each VAULT directly — so it is **retroactive to every sheet printed since Phase 54**. ⛔⛔ **Attacking our own fix found the sharpest defect**: every input to a vault-key wrap is PUBLIC, so a stranger can mint a **perfectly authenticated** envelope ([0048](0048-authenticated-vault-key-envelopes.md) proves WHO, never that they are TRUSTED) and a restore's **empty pin store** made it first-sight TOFU — mutating the fix off returned **five of a stranger's vaults presented as the user's own**. The rule: a vault **named on the sheet** is vouched for by the user; a vault the **index alone** introduced needs a sender in the kit's **own account**, decided **before any network call for that row**. Also: the sheet path was **gated behind the very route it exists to bypass**, one hostile row **erased the report of vaults already restored**, and ⛔ the browsers **revoked a working kit** when a stranger crowded the listing — a denial of the last line of defence under [0040](0040-account-model.md) limitation 1. ⭐ **`sigild`'s BEHAVIOUR is unchanged and the ADR argues why** (a cursor does not help; a scan cap is an [0041](0041-abuse-bounds-and-the-removed-webhook-limiter.md) mistake that would turn a slow-but-CORRECT listing into an EMPTY one; own-account-first only half-closes and nothing could rely on it) — ⚠️ but `sigild` **was** modified by a **comment**, because `maxRecipientIndexRows` justified having no cursor with *"the realistic count is single digits"* and **the count is an attacker's**. ⛔ **Still open:** coverage drifts after the print date, the trust rule defends against a third party and **not against the server**, and the unbounded per-row scan is **pre-existing and not fixed** | Accepted (2026-08) |
