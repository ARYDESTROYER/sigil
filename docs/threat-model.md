...
| AI | ⚠️ **A third party who crowds the kit's discovery listing** (Phase 64, [ADR 0052](decisions/0052-recovery-discovery-and-the-printed-sheet.md)) | Holds an account of their own and knows the kit's device id. Deposits opaque envelopes addressed to it and grants it `read` on vaults they claimed themselves, pushing genuine rows off the single uncursored page of `GET /v1/devices/{deviceID}/keys`. **Measured: 520 vaults in 0.6 s with junk bytes, ~3 s when every envelope is genuine and authenticated.** ⚠️ Reachability is the uncomfortable part: the device id is 128 bits of CSPRNG and unguessable, but `GET /v1/vaults/{id}/grants` discloses it with **`read` alone**, so any current **or former** collaborator on a covered vault keeps it permanently and revocation cannot un-learn it. They can also mint a **genuine, correctly authenticated** envelope to the kit ([ADR 0048](decisions/0048-authenticated-vault-key-envelopes.md) proves *who* sent one, never that they are *trusted*) | ⭐ **Defended CLIENT-SIDE, and the server is deliberately unchanged.** Three rules: a blind restore on `has_more: true` **REFUSES** rather than reporting a partial as complete; the **printed sheet's vault ids** reach each envelope through routes addressed **by vault id**, which nothing can crowd; and the index may only introduce a vault whose **sender is a device in the kit's own account**, so an envelope a stranger put in the LISTING is never fetched, never unwrapped and never pinned. ⚠️ **That last clause is about INDEX-introduced rows ONLY.** A vault id the user TYPES is deliberately exempt from the sender check — the sheet is the user vouching for it — so a typed id belonging to a stranger who granted the kit `read` still reaches first-sight TOFU and IS pinned and unwrapped. That is the design, not an oversight: the alternative refuses a genuine cross-account share the user is deliberately naming ⚠️ **Not defended:** a vault covered **after** the sheet was printed is on no sheet, so under a live flood it stays undiscoverable; and a stranger who deposits **without** granting read causes an unbounded server-side scan — a **latency** denial that leaves the listing correct, pre-existing, and recorded rather than fixed | Client-side, plus the printed sheet |
...
- ✅ ~~**A kit covering more than 500 vaults SILENTLY RECOVERS THE FIRST 500 AND
  REPORTS SUCCESS**~~ — **RETIRED in Phase 64
  ([ADR 0052](decisions/0052-recovery-discovery-and-the-printed-sheet.md))**, and
  the sentence that replaced it is the interesting one. The old text also asserted
  that "**no client in this repo reads it**" and that a **cursor** was the only
  fix; the first had been false since Phase 58 (the JS half refused on the flag),
  and the second is false outright — Phase 64 closed it in Rust with **no cursor
  and no server change**, because the **printed sheet already carries the covered
  vault ids**. Every client now **refuses** a blind restore on a truncated index
  and recovers from those ids instead. ⚠️ What is **not** retired: the sheet lists
  coverage **as of the print date**, so a vault covered afterwards is on no sheet
  and the index remains its only discovery path.
- ⚠️ **RECOVERY DISCOVERY CAN BE DENIED BY A THIRD PARTY — see adversary `AI`
  below.** This is the reason the bullet above changed rather than simply closing.
